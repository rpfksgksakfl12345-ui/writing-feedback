...
export default function LoginPage() {
  ...
  return (
    <section className="mx-auto max-w-3xl overflow-hidden rounded-[28px] border border-[#E8DEC7] bg-paper-base shadow-sm">
      <div className="bg-[radial-gradient(rgba(120,90,50,.05)_1px,transparent_1px)] bg-[length:24px_24px] px-8 py-8">
        <p className="text-sm font-semibold text-teacher-accent">Writing Feedback</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-[#2E2A24]">로그인</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[#5A5247]">
          교사는 이메일로 관리 화면에 들어가고, 학생은 선생님이 나누어 준 학급코드와 번호로 접속합니다.
        </p>
        ...
        <div className="mt-6 inline-flex rounded-md bg-[#F4ECDC] p-1">
          {(
            ...
            return (
              <button
                key={tab.id}
                className={`min-h-10 rounded-md px-5 py-2 text-sm font-semibold shadow-none ${
                  isActive
      ...
      <div className="px-8 pb-9">
        {mode === "TEACHER" ? (
          <form className="mt-6 grid gap-5 rounded-xl border border-[#E8DEC7] bg-[#FFFAF0] p-5 shadow-sm" onSubmit={handleTeacherLogin}>
            ...
            <button
              className="inline-flex min-h-11 items-center justify-center rounded-md bg-teacher-accent px-[18px] py-[13px] text-[15px] font-semibold text-[#FFFAF0] shadow-[0_1px_0_rgba(40,60,90,.15),0_2px_6px_rgba(60,80,120,.18)] hover:bg-teacher-accent/90 disabled:cursor-not-allowed disabled:bg-[#D6CCB3]"
              disabled={loading}
              type="submit"
            >
          ...
          <form className="mt-6 grid gap-5 rounded-xl border border-[#E8DEC7] bg-[#FFFAF0] p-5 shadow-sm" onSubmit={handleStudentLogin}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block text-sm font-semibold text-[#5A5247]">
                학급코드
                <input
                  className="mt-2 h-11 rounded-md border-[#E8DEC7] bg-[#FFFAF0] text-sm uppercase text-[#2E2A24] focus:border-student-accent focus:ring-student-accent/20"
                  value={classCode}
              ...
              <label className="block text-sm font-semibold text-[#5A5247]">
                번호
                <input
                  className="mt-2 h-11 rounded-md border-[#E8DEC7] bg-[#FFFAF0] text-sm text-[#2E2A24] focus:border-student-accent focus:ring-student-accent/20"
                  value={studentNumber}
            ...
            {error ? <p className="text-sm font-semibold text-[#B0533A]">{error}</p> : null}
            <button
              className="inline-flex min-h-11 items-center justify-center rounded-md bg-student-accent px-[18px] py-[13px] text-[15px] font-semibold text-[#FFFAF0] shadow-[0_1px_0_rgba(120,60,30,.15),0_2px_6px_rgba(180,90,50,.18)] hover:bg-student-accent/90 disabled:cursor-not-allowed disabled:bg-[#D6CCB3]"
              disabled={loading}
              type="submit"
            >
              {loading ? "접속 중..." : "학생 로그인"}
